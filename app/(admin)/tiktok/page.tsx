  ...
  return (
    <div className="p-8 max-w-3xl">
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          <h1 className="text-lg font-bold" style={{ fontFamily: 'var(--font-poppins)', color: '#e8eeff', letterSpacing: '-0.02em' }}>TikTok</h1>
          <p className="text-[11px]" style={{ color: '#3d5475' }}>{stats.length} snapshot{stats.length !== 1 ? 's' : ''} logged</p>
        </div>
      </div>
      <SocialStatsPanel platform="tiktok" stats={stats} accentColor={ACCENT} />
    </div>
  )
}
